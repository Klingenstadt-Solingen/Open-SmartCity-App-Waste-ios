...
///  | pygmentize -g
///  ```
class OSCAWasteLocationTests: OSCAParseClassObjectTestSuite<OSCAWasteLocation> {
  override open func makeSpecificObject() -> OSCAWasteLocation? {
    nil
  ...
  /// The file name has to match with the test class name: `OSCAWasteLocationTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `OSCAWasteLocation` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `OSCAWasteLocation` objects in an array from network?
...
}// end class OSCAWasteLocationTests
#endif
