...
///  | pygmentize -g
///  ```
class OSCAWasteAddressTests: OSCAParseClassObjectTestSuite<OSCAWasteAddress> {
  override open func makeSpecificObject() -> OSCAWasteAddress? {
    nil
  ...
  /// The file name has to match with the test class name: `OSCAWasteAddressTests.json`
  override func testJSONDataAvailable() throws -> Void {
    try super.testJSONDataAvailable()
  }// end override func testJSONDataAvailable
#if DEBUG
  /// Is it possible to deserialize `JSON` scheme example data to an array  `OSCAWasteAddress` 's?
  override func testDecodingJSONData() throws -> Void {
    try super.testDecodingJSONData()
  }// end override func testDecodingJSONData
#endif
  /// is it possible to fetch up to 1000 `OSCAWasteAddress` objects in an array from network?
...
}// end class OSCAWasteAddressTests
#endif
